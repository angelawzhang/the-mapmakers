...
<h2>Main Reasons of Migration</h2>
...
<div class="chart" bind:clientWidth={width} bind:clientHeight={height}>
		...
		<g class="axis x-axis">
			{#each points as point (point.reason)}  
				<g class="tick" transform="translate({xScale(point.reason)},{height})">   
					<text x="{barWidth/2}" y="-4">{width > 380 ? point.reason : formatMobile(point.reason)}</text>
				</g> 
			{/each} 
		</g> 
  ...
  >
    {#if hovered !== -1}
        <p>{points[hovered].numOfHouseholds} households main reason of migration was: {points[hovered].reason}</p>
    {/if}
  </div>
</div>
<div>
